#!/usr/bin/env python3
"""
test_scrapingdog.py — self-contained reachability/extraction test for the Scrapingdog
...
HOW TO RUN
  ...
  python test_scrapingdog.py                   # 3 trials per operational domain
  python test_scrapingdog.py --trials 5        # custom trial count
...
ENDPOINT = "https://api.scrapingdog.com"
SCRAPE_PATH = "/scrape"
AUTH_ENV = "SCRAPINGDOG_TOKEN"
...
FROZEN_AVG_COST_PER_1K_USD = 1.51
...
DOMAINS = {
    ...
    "booking.com": {
        ...
        "tier": {"params": {"premium": "true"}, "endpoint_override": None},
        "verify_keys": [
            "hp_hotel_name",
        ...
        "need_at_least": 2,
        "cost_per_1k_usd": None,  # frozen slice has null cost for booking
    },
    "capterra.com": {
        "status": "operational",
    ...
    "google.com": {
        ...
        "need_at_least": 2,
        "cost_per_1k_usd": None,  # frozen slice has null cost for google
    },
    "idealista.com": {
        "status": "operational",
        "url": "https://www.idealista.com/inmueble/110715434/",
        "tier": {"params": {"premium": "true", "country": "es"}, "endpoint_override": None},
        "verify_keys": [
    ...
    "linkedin.com": {
        ...
        "need_at_least": 2,
        "cost_per_1k_usd": None,  # frozen slice has null cost for linkedin
    },
    "reddit.com": {
        "status": "operational",
        ...
        "need_at_least": 2,
        "cost_per_1k_usd": None,  # frozen slice has null cost for reddit
    },
    "tripadvisor.com": {
        "status": "operational",
        ...
        "tier": {"params": {"premium": "true", "dynamic": "true"}, "endpoint_override": None},
        "verify_keys": [
            "Fairmont",
            "THE PLAZA NEW YORK",
    ...
    "trustpilot.com": {
        ...
        "tier": {"params": {"premium": "true", "dynamic": "true", "wait": 15000}, "endpoint_override": None},
        "verify_keys": [
    ...
    "walmart.com": {
        ...
        "need_at_least": 2,
        "cost_per_1k_usd": None,  # frozen slice has null cost for walmart
    },
    "x.com": {
    ...
    "zillow.com": {
        ...
        "tier": {"params": {"dynamic": "true"}, "endpoint_override": None},
        "verify_keys": [
...
def build_request(domain_cfg, api_key):
    """Build a Scrapingdog GET request per the slice request_recipe.
    ...
    params (empty in this slice). Returns a urllib.request.Request (GET).
    """
    tier = domain_cfg["tier"]
    base = tier["endpoint_override"] or (ENDPOINT + SCRAPE_PATH)
    params = {"api_key": api_key, "url": domain_cfg["url"]}
    # Tier params are literal strings ("true"/"es") or ints (wait); urlencode stringifies.
    params.update(tier["params"])
    query = urllib.parse.urlencode(params)
    sep = "&" if "?" in base else "?"
    full_url = base + sep + query
    return urllib.request.Request(full_url, method="GET")


def unwrap(raw_bytes):
    """Scrapingdog returns raw HTML or JSON.

    Decode as UTF-8 (lenient). If the body parses as JSON (dedicated endpoints can
    return JSON), stringify it so the verify substrings still match against the text.
    Otherwise return the raw HTML body unchanged.
    """
    ...
    if stripped[:1] in ("{", "["):
        try:
            ...
            # Compact separators (no spaces) so verify_keys like '"username":"nike"'
            # match — this mirrors JS JSON.stringify, keeping both languages aligned.
...
def main():
    parser = argparse.ArgumentParser(description="Scrapingdog per-domain test (frozen 2026 baseline).")
    ...
    print("Scrapingdog test — {} trials/domain, pacing {}s between requests\n".format(trials, PACE_SECONDS))
