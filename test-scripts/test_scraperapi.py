...
HOW TO RUN
  export SCRAPERAPI_TOKEN=<your_key>          # required; without it the script
                                              # prints a hint and exits 0
  python3 test_scraperapi.py                  # default 3 trials per domain
  python3 test_scraperapi.py --trials 5       # override trial count

  Stdlib only (urllib, json, os, sys, base64, argparse, time). Python 3.8+.
  No third-party packages, no shared imports across providers.

WHAT IT WRITES
  ../../test-results/scraperapi.run.json  (relative to this script's own dir,
...
PROVIDER QUIRKS BAKED IN (from the slice request_recipe + quirks)
  ...
  - Real credit cost is in the sa-credit-cost response header and often differs
    from doc cost (Amazon 5cr, Google 25cr, LinkedIn 30cr, YouTube 10cr, ...).
    We do NOT bill on it; we just capture cost from the frozen slice.
  - T5 ultra_premium is expensive and slow; trials are capped (see MAX_TRIALS).
...
AUTH_ENV = "SCRAPERAPI_TOKEN"
REPORT_DATE = "2026-06-08"  # today (frozen-compatible)

# T5 ultra_premium is expensive + slow per the recipe note; cap trials so a
# user does not burn a pile of credits on the heavy tiers.
MAX_TRIALS = 8

# avg_cost_per_1k_usd from the frozen slice summary; cost is NOT measured live.
FROZEN_AVG_COST_PER_1K_USD = 12.04
...
DOMAINS = {
    ...
    "bestbuy.com": {
        ...
        "tier": {"params": {"ultra_premium": "true", "render": "true"}, "endpoint_override": None},
    ...
    "bing.com": {
        ...
        "tier": {"params": {"ultra_premium": "true"}, "endpoint_override": None},
    ...
    "booking.com": {
        ...
        "tier": {"params": {"render": "true"}, "endpoint_override": None},
    ...
    "g2.com": {
        ...
        "tier": {"params": {"ultra_premium": "true", "render": "true",
                            "wait_for_selector": ".product-head"}, "endpoint_override": None},
    ...
    "reddit.com": {
        ...
        "tier": {"params": {"premium": "true", "render": "true"}, "endpoint_override": None},
...
def build_request(target_url, params):
    ...
    Transport GET. Auth = querystring api_key. Query shape:
      api_key=<KEY>, url=<targetUrl>, plus tier params.
    Base endpoint https://api.scraperapi.com (the "/?api_key=&url=" form).
    Returns RAW HTML — no envelope.
    """
    token = os.environ[AUTH_ENV]
    query = {"api_key": token, "url": target_url}
    # Tier params (render, premium, ultra_premium, country_code, wait_for_selector).
    query.update(params)
...
def unwrap(raw_bytes):
    """Response unwrapping for ScraperAPI: the body is RAW HTML, no JSON
    envelope. Decode bytes -> str (utf-8, replace errors)."""
...
def run_trial(target_url, params):
    """Single request. Returns (ok_http, body_or_empty, latency_ms)."""
    ...
    try:
        ...
        return True, unwrap(raw), latency_ms
...
def main():
    parser = argparse.ArgumentParser(description="ScraperAPI provider test (frozen 2026 baseline).")
    ...
    if trials > MAX_TRIALS:
        ...
        print(f"[note] capping --trials {trials} -> {MAX_TRIALS} (ultra_premium is costly)")
        trials = MAX_TRIALS
    ...
    for domain, cfg in DOMAINS.items():
        ...
        operational_count += 1
        params = cfg["tier"]["params"]
        passes = 0
        latencies = []
        for t in range(trials):
            ok_http, body, latency_ms = run_trial(cfg["url"], params)
            latencies.append(latency_ms)
            if ok_http and body:
                passed, _hits = verify(body, cfg["verify_keys"], cfg["need_at_least"])
                if passed:
                    passes += 1
